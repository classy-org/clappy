...
describe(`DELETE (${MODE} mode)`, () => {
  ...
  describe('Cross-environment request checks (with --prod flag)', () => {
    ...
    ts.forEach((apiId, envId, grantType, prefix) => {
      ...
      beforeAll(async () => {
        if (MODE === 'static') {
          ({ requests, response } = await ts.run(
            `use ${apiId} ${envId} ${grantType}; state enableProdModifications true; delete ${v.REQ_PATH}`
          ));
      ...
      test(`${prefix} Check API request`, () => {
        ...
        const expected = ts.apiRequestMatcher(apiId, envId, grantType, 'delete');
        expect(apiRequest).toMatchObject(expected);
      });

      test(`${prefix} Check Clappy response`, () => {
        expect(response).toEqual({ mock: 'response', for: 'DELETE' });
      });
    ...
    beforeAll(async () => {
      if (MODE === 'static') {
        ({ requests, resolved, error } = await ts.run(
          `use foo prod cc; delete ${v.REQ_PATH}`
        ));
      }
      else {
  ...
  describe('When dryRun is true', () => {
    ...
    beforeAll(async () => {
      if (MODE === 'static') {
        ({ requests, resolved, error } = await ts.run(
          `use foo prod cc; state dryRun true; delete ${v.REQ_PATH}`
        ));
      }
      else {
        ({ requests, resolved, error } = await ts.run((clappy) => {
    ...
    beforeAll(async () => {
      ...
      if (MODE === 'static') {
        ({ requests } = await ts.run(
          `use local foo cc; state token client_credentials.value "TEST_TOKEN"; state token client_credentials.expires ${expiration}; delete ${v.REQ_PATH}`
        ));
      }
      else {
