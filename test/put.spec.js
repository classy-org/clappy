...
describe(`PUT (${MODE} mode)`, () => {
  ...
  describe('Cross-environment request checks (with --prod flag)', () => {
    ...
    ts.forEach((apiId, envId, grantType, prefix) => {
      ...
      beforeAll(async () => {
        if (MODE === 'static') {
          ({ requests, response } = await ts.run(
            `use ${apiId} ${envId} ${grantType}; state enableProdModifications true; put ${v.REQ_PATH} ${JSON.stringify(v.REQ_BODY)}`
          ));
      ...
      test(`${prefix} Check API request`, () => {
        ...
        const expected = ts.apiRequestMatcher(apiId, envId, grantType, 'put');
        expect(apiRequest).toMatchObject(expected);
      });

      test(`${prefix} Check Clappy response`, () => {
        expect(response).toEqual({ mock: 'response', for: 'PUT' });
      });
    ...
    beforeAll(async () => {
      if (MODE === 'static') {
        ({ requests, resolved, error } = await ts.run(
          `use foo prod cc; put ${v.REQ_PATH} ${JSON.stringify(v.REQ_BODY)}`
        ));
      }
      else {
  ...
  describe('When dryRun is true', () => {
    ...
    beforeAll(async () => {
      if (MODE === 'static') {
        ({ requests, resolved, error } = await ts.run(
          `use foo prod cc; state dryRun true; put ${v.REQ_PATH} ${JSON.stringify(v.REQ_BODY)}`
        ));
      }
      else {
        ({ requests, resolved, error } = await ts.run((clappy) => {
    ...
    beforeAll(async () => {
      ...
      if (MODE === 'static') {
        ({ requests } = await ts.run(
          `use local foo cc; state token client_credentials.value "TEST_TOKEN"; state token client_credentials.expires ${expiration}; put ${v.REQ_PATH} ${JSON.stringify(v.REQ_BODY)}`
        ));
      }
      else {
